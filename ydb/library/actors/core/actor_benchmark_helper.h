...
struct TTestEndDecorator : TDecorator {
        ...
        , ActorsAlive(actorsAlive)
    {
        ...
        ACTORLIB_DEBUG(EDebugLevel::Trace, "TTestEndDecorator::TTestEndDecorator: alive ", x);
    }

    virtual ~TTestEndDecorator() {
        auto alive = AtomicDecrement(*ActorsAlive);
        ACTORLIB_DEBUG(EDebugLevel::Trace, "TTestEndDecorator::~TTestEndDecorator: alive ", alive);
        if (alive == 0) {
            Pad->Unpark();
        }
    }
...
struct TActorBenchmark {
    ...
    public:
        ...
        virtual ~TSendReceiveActor() {
            ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::~TSendReceiveActor: ", this->SelfId());
        }
        ...
        void Bootstrap(const TActorContext &ctx) {
            ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::Bootstrap: ", this->SelfId());
        ...
        void SpecialSend(TAutoPtr<IEventHandle> ev, const TActorContext &ctx, bool own) {
            ...
            if (own) {
                --OwnEventsCounter;
                ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::SpecialSend: own; ", OwnEventsCounter + 1, " -> ", OwnEventsCounter);
            }
            if (ToSchedule) {
        ...
        void Stop() {
            ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::Stop: ", this->SelfId());
            if (SharedCounters && IsLeader) {
        ...
        void PassAway() override {
            ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::PassAway: ", this->SelfId());
            TBase::PassAway();
        }

        bool CheckWorkIsDone() {
            if (OwnEventsCounter || OtherEventsCounter || EndlessSending) {
                ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::CheckWorkIsDone: ", this->SelfId(), " OwnEventsCounter: ", OwnEventsCounter, " OtherEventsCounter: ", OtherEventsCounter, " EndlessSending: ", EndlessSending);
                return false;
            }
            Stop();
            return true;
        ...
        STFUNC(StateFunc) {
            ACTORLIB_DEBUG(EDebugLevel::Trace, "TSendReceiveActor::StateFunc: ", this->SelfId());
            ++EventsCounter;
