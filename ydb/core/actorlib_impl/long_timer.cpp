...
class TLongTimer : public TActor<TLongTimer> {
    ...
    void Round(const TActorContext &ctx) {
        ...
        if (delta <= TDuration::Seconds(ThresholdSec)) {
            ctx.Schedule(SignalTime, Ev.Release(), Cookie.Release());
            return Die(ctx);
        }
...
public:
        ...
        const TActorId& parentId)
    {
        ...
        const TActorId timerActorID = ctx.Register(new TLongTimer(now, signalTime, ev, cookie), parentId, TMailboxType::HTSwap, poolId);
